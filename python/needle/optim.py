...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for para in self.params:
            ...
            v = self.v.get(para)
            grad = para.grad.detach() + self.weight_decay * para.detach()
            m_new = (1 - self.beta1) * grad + self.beta1 * (0 if m == None else m)
            v_new = (1 - self.beta2) * (grad**2) + self.beta2 * (0 if v == None else v)
            self.m[para] = m_new
            self.v[para] = v_new
            ...
            para.cached_data -= (
                self.lr * ((m_new / ((v_new**0.5) + self.eps))).cached_data
            )
        ### END YOUR SOLUTION
