    ...
    def forward(self, X, h0=None):
        ...
        if h0 is None:
            h0 = init.zeros(
                len(self.rnn_cells),
                X.shape[1],
                self.rnn_cells[0].W_hh.shape[0],
                device=X.device,
                dtype=X.dtype,
            )
        h_input = list(
            ops.split(h0, 0)
        )  # list length = num_layers, element shape = (bs, hidden_size)
        X_input = list(
            ops.split(X, 0)
        )  # list length = seq_len, element shape = (bs, input_size)
        ...
        output = ops.stack(
            X_input, 0
        )  # output features of last layer == input X of last+1 layer
...
class LSTMCell(Module):
    def __init__(
        ...
        self.bias_ih = (
            Parameter(
                init.rand(
                    4 * hidden_size, low=-bound, high=bound, device=device, dtype=dtype
                )
            )
            if bias
            else None
